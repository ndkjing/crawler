...
# 获取页面下所有主题url
def parser_topic_page(pages_urls_q,topics_url_q):
    print('parser_topic')
    page_url = pages_urls_q.get()

    Page_Obj = down(page_url)
    ## 若是下载失败返回False
...
if __name__ == '__main__':
    ...
    ## 定义线程池
    pool_size = 1
    parser_topic_pool = Pool(pool_size)
    parser_image_pool = Pool(pool_size)
    download_image_pool = Pool(pool_size)
    #  获取页面下主题
    while True:
        ## 获取所有页面
        parser_topic_pool.apply_async(func=parser_topic_page, args=(pages_urls_q,topics_url_q))

        # 获取主题下图片
        parser_image_pool.apply_async(func=parser_image_url,args=(topics_url_q,))

        # 下载图片
        parser_image_pool.apply_async(func=download_image,args=(images_url_q,))
