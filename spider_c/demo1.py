...
def down(url,encode='gbk',dead_request=False):
    """
    下载指定url内容
...
def parser_topic_page():
    ...
    offset = 101  # 页面偏移
    with open('topic_save.json','r') as f:
    ...
    while offset < page:  # 主题列表分页数
        ...
        Page_Obj = down(PageList,dead_request=True)
...
def parser_image_url():
    with open('topic_save.json') as f:
        topics = json.load(f)
    title_urls = []
    for topic in topics:
        ...
        if len(title_urls)%10:
            with open('title_urls_0629.json','w') as f:
